...
[azErrorDeg,newIdx] = calcLocalizationError(azRef,azEst);

% Percentage of correctly localized sound sources wihtin "thresDeg"
pcCorrect = 100 * sum(azErrorDeg <= thresDeg)/numel(azRef); 

% RMSE of all sound source positions
rmse = sqrt(mean(azErrorDeg.^2));
